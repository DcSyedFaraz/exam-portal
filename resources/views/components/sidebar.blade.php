<aside id="sidebar" class="w-64 bg-[#1A1A2E] flex flex-col h-full fixed lg:static inset-y-0 left-0 z-50 -translate-x-full lg:translate-x-0 transition-transform duration-300">
    ...
    <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
        ...
        @if(auth()->user()->hasRole('admin'))
            ...
            {{-- <p class="text-gray-500 text-xs uppercase tracking-widest px-4 pt-4 pb-1">Tools</p>

            <a href="{{ route('admin.terminal') }}"
               class="{{ request()->routeIs('admin.terminal*') ? 'sidebar-link-active' : 'sidebar-link' }}">
    ...
    <div class="mx-4 mb-4 rounded-xl bg-yellow-400/10 border border-yellow-400/20 px-4 py-3 text-xs text-gray-300 space-y-1.5">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="tel:+255744950150" class="hover:text-yellow-400 transition-colors">+255 744 950 150</a>
        </div>
        <div class="flex items-center gap-2">
            <svg class="w-3.5 h-3.5 text-yellow-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
