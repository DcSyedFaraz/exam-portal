...
<div class="max-w-3xl mx-auto">
    ...
    {{-- Sticky Exam Header --}}
    <div class="sticky top-0 bg-white border border-gray-100 rounded-xl shadow-sm z-30 px-6 py-4 mb-6 flex items-center justify-between">
        <div>
    ...
    <form id="exam-form"
        ...
        <div class="space-y-6">
            ...
            <div class="card question-card" id="qcard-{{ $question->id }}" data-question-id="{{ $question->id }}">
                <div class="flex items-start gap-3 mb-4">
                    <span class="bg-yellow-400 text-gray-900 font-bold text-sm w-7 h-7 rounded-full flex items-center justify-center shrink-0">
                        {{ $i + 1 }}
                    </span>
                    <div class="flex-1">
                ...
                <div class="space-y-2">
                    ...
                    <label class="flex items-center gap-3 p-3 rounded-xl border border-gray-200 cursor-pointer hover:border-yellow-400 hover:bg-yellow-50 transition-all answer-label"
                           for="opt-{{ $option->id }}">
                        <input type="radio"
                               id="opt-{{ $option->id }}"
                        ...
                        <span class="w-7 h-7 rounded-full bg-gray-100 text-gray-600 text-xs font-bold flex items-center justify-center shrink-0">
                            {{ chr(65 + $j) }}
                        </span>
                ...
                <div class="flex gap-4">
                    ...
                    <label class="flex-1 flex items-center justify-center gap-2 p-4 rounded-xl border-2 border-gray-200 cursor-pointer hover:border-yellow-400 hover:bg-yellow-50 transition-all answer-label font-semibold text-gray-700"
                           for="opt-{{ $option->id }}">
                        <input type="radio"
                               id="opt-{{ $option->id }}"
                               name="answers[{{ $question->id }}]"
                ...
                <div class="space-y-3">
                    @php
                        // Build shuffled right column options
                        $rightOptions = $question->options->pluck('match_pair')->shuffle()->unique()->values();
                    @endphp
                    @foreach($question->options as $option)
                    <div class="flex items-center gap-4 p-3 rounded-xl bg-gray-50">
                        <span class="text-sm font-medium text-gray-800 flex-1">{{ $option->option_text }}</span>
    ...
    <div class="card mt-6">
        ...
        <div class="flex items-center gap-4 mt-3 text-xs text-gray-500">
            <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded bg-yellow-400 inline-block"></span> Answered</span>
            <span class="flex items-center gap-1.5"><span class="w-3 h-3 rounded bg-gray-200 inline-block"></span> Unanswered</span>
        </div>
    </div>
</div>
...
function updateProgress() {
    ...
    document.getElementById('submit-status').textContent =
        count === totalQuestions ? 'All questions answered. Ready to submit!' : `${totalQuestions - count} question(s) unanswered.`;
}
...
function scrollToQuestion(id) {
    document.getElementById('qcard-' + id)?.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
