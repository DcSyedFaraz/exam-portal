...
@section('breadcrumb', 'Admin › Exams › ' . $exam->title . ' › ' . (isset($question) ? 'Edit Question' : 'Add
    Question'))
...
@section('content')
    @php
        ...
        $action = $isEdit ? route('admin.questions.update', $question) : route('admin.exams.questions.store', $exam);
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h2 class="text-lg font-bold font-heading text-gray-900">{{ $isEdit ? 'Edit Question' : 'Add New Question' }}
                </h2>
            ...
            'ai_evaluated' => ['label' => 'Open Ended (AI)', 'icon' => '🤖', 'ai' => true],
                            ...
                            <label class="type-card cursor-pointer" onclick="selectTypeCard(this)">
                                <input type="radio" name="question_type" value="{{ $val }}"
                                    {{ $isSelected ? 'checked' : '' }} onchange="onTypeChange('{{ $val }}')"
                                    class="sr-only">
                                <div
                                    class="type-card-inner flex flex-col items-center justify-center gap-1 p-3 rounded-xl border-2 transition text-center relative
                                    {{ $isSelected ? 'border-yellow-400 bg-yellow-50 shadow-sm' : 'border-gray-200 bg-white hover:border-yellow-300' }}">
                                    <span class="text-xl">{{ $meta['icon'] }}</span>
                                    <span
                                        class="text-xs font-medium leading-tight {{ $isSelected ? 'text-yellow-700' : 'text-gray-600' }}">{{ $meta['label'] }}</span>
                                    @if ($meta['ai'])
                                        <span
                                            class="absolute top-1 right-1 text-[9px] bg-purple-100 text-purple-600 font-semibold px-1 rounded">AI</span>
                                    @endif
                                </div>
                            </label>
                        @endforeach
                ...
                <div class="mb-5 w-36">
                    ...
                    <input type="number" name="marks" value="{{ old('marks', $question->marks ?? 1) }}" min="1"
                        class="form-input">
                </div>

                {{-- Shared question text (MCQ, True/False, Match, AI) --}}
                <div id="wrap-question-text"
                    class="mb-5 {{ in_array($type, ['fill_blank', 'word_bank', 'picture']) ? 'hidden' : '' }}">
                    <label class="form-label">Question Text <span class="text-red-500">*</span></label>
                    <textarea name="question_text" rows="3" class="form-input" placeholder="Enter the question...">{{ old('question_text', $question->question_text ?? '') }}</textarea>
                </div>

                {{-- ── MCQ ────────────────────────────────────────────────────────── --}}
                <div id="options-mcq" class="{{ $type !== 'mcq' ? 'hidden' : '' }} mb-5">
                    <label class="form-label">Options <span class="text-xs text-gray-400">(select the correct
                            answer)</span></label>
                    <div class="space-y-2" id="mcq-options-container">
                        @php
                            ...
                            if ($oldOptions) {
                                $mcqOptions = collect($oldOptions)->map(
                                    fn($o) => (object) [
                                        'option_text' => $o['text'] ?? '',
                            ...
                            @php
                                ...
                                $isChecked =
                                    $oldCorrectMcq !== null
                                        ...
                                        : $opt && $opt->is_correct;
                                if ($oldCorrectMcq === null && !$mcqOptions->count() && $i === 0) {
                                    $isChecked = true;
                                }
                            @endphp
                            <div class="flex items-center gap-3 mcq-row">
                                <input type="radio" name="correct_option_mcq" value="{{ $i }}"
                                    {{ $isChecked ? 'checked' : '' }} class="w-4 h-4 text-yellow-400">
                                <input type="text" name="options[{{ $i }}][text]"
                                    value="{{ $opt->option_text ?? '' }}" placeholder="Option {{ chr(65 + $i) }}"
                                    class="form-input flex-1">
                                <input type="hidden" name="options[{{ $i }}][is_correct]" value="0"
                                    class="mcq-is-correct">
                            </div>
                        @endfor
                    </div>
                    <button type="button" onclick="addMcqOption()" class="text-sm text-yellow-600 hover:underline mt-2">+
                        Add option</button>
                </div>
                ...
                <div id="options-true_false" class="{{ $type !== 'true_false' ? 'hidden' : '' }} mb-5">
                    ...
                    @php
                        if (old('tf_correct') !== null) {
                            ...
                            $tfCorrect = strtolower(
                                $question->options->firstWhere('is_correct', true)?->option_text ?? 'true',
                            );
                        } else {
                            $tfCorrect = 'true';
                        }
                    ...
                    <div class="flex gap-6 mt-1">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="tf_correct" value="true"
                                {{ $tfCorrect === 'true' ? 'checked' : '' }} class="w-4 h-4 text-yellow-400">
                            <span class="text-sm font-medium">True</span>
                        </label>
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="radio" name="tf_correct" value="false"
                                {{ $tfCorrect === 'false' ? 'checked' : '' }} class="w-4 h-4 text-yellow-400">
                ...
                <div id="options-match" class="{{ $type !== 'match' ? 'hidden' : '' }} mb-5">
                    ...
                    <div class="space-y-2" id="match-pairs-container">
                        @php
                            ...
                            if ($oldMatch) {
                                $matchRows = collect($oldMatch)->map(
                                    fn($o) => (object) [
                                        'option_text' => $o['text'] ?? '',
                        ...
                        @foreach ($matchRows as $idx => $row)
                            <div class="flex items-center gap-2 match-row">
                                <input type="text" name="options[{{ $idx }}][text]"
                                    value="{{ $row->option_text }}" placeholder="Left item" class="form-input flex-1">
                                <span class="text-gray-400 shrink-0">→</span>
                                <input type="text" name="options[{{ $idx }}][match_pair]"
                                    value="{{ $row->match_pair }}" placeholder="Right item" class="form-input flex-1">
                ...
                <div id="options-fill_blank" class="{{ $type !== 'fill_blank' ? 'hidden' : '' }} mb-5 space-y-4">
                    ...
                    <div>
                        <label class="form-label">Instructions for Students <span
                        ...
                        <input type="text" name="fill_blank_instructions"
                            ...
                            placeholder="e.g. Fill in the blanks with the correct word." class="form-input w-full">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            <input type="text" name="fb_question_text" id="fill-blank-question"
                                ...
                                placeholder="The capital of Tanzania is ______" oninput="fbUpdatePreview()"
                                class="form-input flex-1">
                        ...
                        <p class="text-xs text-gray-400 mt-1">Click <strong>+ Blank</strong> to insert a blank at cursor,
                            or type <code>______</code> manually.</p>
                    </div>

                    {{-- Live preview with inline answer boxes --}}
                    <div id="fb-preview-wrap" class="hidden">
                        <label class="form-label">Correct Answers
                            <span class="text-xs font-normal text-gray-400 ml-1">— type the answer into each box below
                                (this is how students will see it)</span>
                        ...
                        @endphp
                        <div id="fb-preview" data-saved-answers="{{ json_encode($fbSavedParts) }}"
                ...
                <div id="options-picture" class="{{ $type !== 'picture' ? 'hidden' : '' }} mb-5 space-y-4">
                    <div
                        class="flex items-start gap-2 p-3 bg-purple-50 border border-purple-200 rounded-xl text-xs text-purple-700">
                        <span class="text-base shrink-0">🤖</span>
                        <span>Each sub-question is <strong>graded by AI</strong> — Gemini compares the student's answer
                            against your model answer and awards marks. Partial credit is possible.</span>
                    </div>
                    <div>
                        <label class="form-label">Caption <span class="text-xs text-gray-400">(optional — shown above the
                                image)</span></label>
                                ...
                                <div class="flex gap-2 items-start p-3 bg-gray-50 rounded-lg flex-wrap picture-sub-row">
                                    <select name="sub_labels[]" class="form-input w-16 text-sm">
                                        @foreach (range('a', 'z') as $lbl)
                                            <option {{ $sr['label'] === $lbl ? 'selected' : '' }}>{{ $lbl }}
                                            </option>
                                        @endforeach
                                    </select>
                ...
                <div id="options-word_bank" class="{{ $type !== 'word_bank' ? 'hidden' : '' }} mb-5 space-y-5">
                    ...
                    {{-- How it works info --}}
                    <div
                        class="flex items-start gap-2 p-3 bg-cyan-50 border border-cyan-200 rounded-xl text-xs text-cyan-800">
                        <span class="text-base shrink-0">📚</span>
                        <span>Students see a box of words and must drag or select the correct word into each blank. Write
                            each sentence with <code class="bg-cyan-100 px-1 rounded">___</code> where the blank goes, then
                            pick the correct word from your word bank.</span>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 items-center">
                            <input type="text" name="word_bank_items" id="wb-words-input"
                                value="{{ $wbItems }}" placeholder="e.g. Ocean, Forest, Desert, Farm"
                                oninput="wbUpdatePreview()" class="form-input flex-1">
                        </div>
                        <p class="text-xs text-gray-400 mt-1">Separate words with commas. These appear as chips the student
                            picks from.</p>
                    ...
                    <div>
                        <label class="form-label">Sentences <span class="text-red-500">*</span>
                            <span class="text-xs font-normal text-gray-400 ml-1">— one per row · write the sentence and
                                choose the correct word</span>
                        </label>
                        <div class="space-y-2" id="word-bank-items-container">
                            @php
                                $oldWbOpts = old('options');
                                if ($oldWbOpts) {
                                    $wbRows = collect($oldWbOpts)->map(
                                        fn($o) => (object) [
                            ...
                            @foreach ($wbRows as $wi => $wrow)
                                <div
                                    class="wb-sentence-row flex gap-2 items-center p-3 bg-gray-50 rounded-xl border border-gray-100">
                                    <span
                                        class="text-xs font-bold text-gray-400 w-5 shrink-0 text-center">{{ $wi + 1 }}</span>
                                    <input type="text" name="options[{{ $wi }}][statement]"
                                        value="{{ $wrow->statement }}" placeholder="A fish lives in the ___."
                                    ...
                                    <button type="button"
                                        onclick="this.closest('.wb-sentence-row').remove(); wbRenumber()"
                ...
                <div id="options-ai_evaluated" class="{{ $type !== 'ai_evaluated' ? 'hidden' : '' }} mb-5 space-y-4">
                    <div
                        class="flex items-start gap-2 p-3 bg-purple-50 border border-purple-200 rounded-xl text-xs text-purple-700">
                        <span class="text-base shrink-0">🤖</span>
                        <span>Student answers are <strong>graded by AI</strong> — Gemini compares the student's response
                            against your model answer and awards marks.</span>
                    </div>
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Students will not see this — it is only used by AI for
                            grading.</p>
                    </div>
                </div>
        ...
        function fbUpdatePreview() {
            ...
            val.split(/_{4,}/).forEach((segment, segIdx) => {
                ...
                if (segIdx < val.split(/_{4,}/).length - 1) {
                    ...
                    inp.className =
                        'inline-block w-32 px-3 py-1 border-b-2 border-yellow-400 bg-yellow-50 rounded-lg text-sm font-medium text-gray-900 text-center focus:outline-none focus:border-yellow-600 focus:bg-yellow-100 transition mx-1';
        ...
        function wbUpdatePreview() {
            ...
            if (preview) {
                ...
                preview.innerHTML = words.length ?
                    words.map(w =>
                        `<span class="px-3 py-1 bg-cyan-100 border border-cyan-300 rounded-full text-sm font-medium text-cyan-800">${w}</span>`
                        ).join('') :
